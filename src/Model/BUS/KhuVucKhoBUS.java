...
public class KhuVucKhoBUS {
    ...
    public boolean add(KhuVucKhoDTO kvk) {
        if (isTenKhuVucTonTai(kvk.getTenkhuvuc())) {
    ...
    public boolean isTenKhuVucTonTai(String tenKhuVuc) {
        tenKhuVuc = tenKhuVuc.trim().toLowerCase();
        for (KhuVucKhoDTO kvk : listKVK) {
            if (kvk.getTenkhuvuc().trim().toLowerCase().equals(tenKhuVuc)) {
                return true;
            }
        }
        return false;
