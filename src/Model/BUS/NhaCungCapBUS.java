...
public class NhaCungCapBUS {
    ...
    public boolean add(NhaCungCapDTO ncc) {
        if (isTenNccTonTai(ncc.getTenncc())) {
    ...
    public boolean isTenNccTonTai(String tenNcc) {
        tenNcc = tenNcc.trim().toLowerCase(); // chuẩn hóa để so sánh không phân biệt hoa thường
        for (NhaCungCapDTO ncc : listNcc) {
            if (ncc.getTenncc().trim().toLowerCase().equals(tenNcc)) {
                return true;
            }
        }
        return false;
