...
import View.Dialog.PhanQuyenDialog;
...
public class PhanQuyen extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == mainFunction.btn.get("create")) {
            ...
            int index = this.getRowSelected();
            if (index >= 0) {
                int input = JOptionPane.showConfirmDialog(null,"Bạn có chắc chắn muốn xóa nhà cung cấp!", "Xác nhận xóa nhà cung cấp",JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
                if (input == 0) {
                    nhomquyenBUS.delete(listnhomquyen.get(index));
                    loadDataTalbe(listnhomquyen);
                    JOptionPane.showConfirmDialog(null,"Xóa nhóm quyền thành công!", "Xóa nhà cung cấp",JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
                }
            }
